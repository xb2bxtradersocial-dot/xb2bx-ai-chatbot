...
export default function Settings() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (newKey.trim()) patch.anthropic_api_key = newKey.trim();
    ...
    <div>
      ...
      <div className="settings-cards">
        ...
        <section className="card">
          ...
          <Field label="API key" hint={s.anthropic_api_key_set ? `Current: ${s.anthropic_api_key} — type a new key to replace it.` : 'No key set yet — paste your Anthropic API key.'}>
            <input className="input mono" type="password" placeholder={s.anthropic_api_key_set ? '•••••••••• (unchanged)' : 'sk-ant-api03-…'} value={newKey} onChange={(e) => setNewKey(e.target.value)} />
          </Field>
          <Field label="Main model"><input className="input" value={s.anthropic_model || ''} onChange={(e) => set('anthropic_model', e.target.value)} placeholder="claude-sonnet-4-20250514" /></Field>
          <Field label="Router model"><input className="input" value={s.anthropic_router_model || ''} onChange={(e) => set('anthropic_router_model', e.target.value)} placeholder="claude-sonnet-4-20250514" /></Field>
          <div className="row-2">
